<x-emails.email-container>
    <x-emails.email-body>
        ...
        @php
            ...
            $url = config('app.url');
            $approved = true;
            $reason = 'No se encontró el usuario en la base de datos.';
        @endphp

        @if ($approved)
            <p>
                Estimado usuario, tu solicitud de registro en {{ $appName }} ha sido
                <strong class="color-finalizado">aprobada</strong>
                . Puedes comenzar a hacer uso de la aplicación desde el siguiente enlace
            </p>
